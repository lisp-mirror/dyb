...
(defclass fb-post-form (ajax-widget)
  ((grid :initarg :grid
         :initform nil
         :accessor grid)
   (message :initarg :message))
...
(defmethod render ((widget fb-post-form) &key )
  (let* ((comment-form (make-widget 'html-simple-framework-form 
                                    :name "facebook-post-form"
                                    :grid-size 12
                                    :form-id "facebook-post-form"
...
(defmethod handle-action ((grid generic-grid) (action (eql 'new)))
  (setf (action-widget grid)
        (make-widget 'fb-post-form 
                     :grid grid 
                     :name "facebook-post-action-form")))
...
(defmethod action-handler ((widget fb-post-form))

  (when (string-equal (parameter "action") "post-facebook")  
