...
(defun make-message--tag (message--tag)
	   (make-instance 'message--tag :id (get-message--tag-id message--tag) :name (get-message--tag-name message--tag) :offset (get-message--tag-offset message--tag) :length (get-message--tag-length message--tag) :type (get-message--tag-type message--tag)))

(defun make-message--tag-list (message--tags)
	   (let(( out '()))
	   (dolist (st message--tags) (setf out (append out (list (make-message--tag (first (rest st))))))) out))
...
(defun make-property (property-raw)
	   (make-instance 'property :name (get-property-name property-raw) :text (get-property-text property-raw)))

(defun make-property-list (properties-raw)
	   (let(( out '()))
	   (dolist (prop properties-raw) (setf out (append out (list (make-property prop))))) out))
...
(defun make-action-list (post)
	   (let(( out '()))
	   (dolist (act (get-actions post)) (setf out (append out (list (make-instance 'action :name (get-action-name act) :link (get-action-link act)))))) out ))
...
(defun make-privacy (post)
	   (make-instance 'privacy :description (get-privacy-description post) :value (get-privacy-value post)))
...
(defun make-like-list (post)
	   (let(( out '()))
	   (dolist (lk (get-likes-data post)) (setf out (append out (list (make-instance 'like :name (get-like-name lk) :id (get-like-id lk)))))) out ))
...
(defun make-location (location)
	   (make-instance 'location :latitude (get-location-latitude location) :longitude (get-location-longitude location)))

(defun make-place (place)
	   (make-instance 'place :id (get-place-id place) :name (get-place-name place) :location (make-location (get-place-location place))))
...
(defun make-story--tag (story--tag)
	   (make-instance 'story--tag :id (get-story--tag-id story--tag) :name (get-story--tag-name story--tag) :offset (get-story--tag-offset story--tag) :length (get-story--tag-length story--tag) :type (get-story--tag-type story--tag)))
...
(defun make-with--tag (tag-element)
	   (make-instance 'with--tag :id (get-with--tag-id tag-element) :name (get-with--tag-name tag-element)))

(defun make-with--tag-list (with--tags)
	   (let(( out '()))
	   (dolist (wt with--tags) (setf out (append out (list (make-with--tag wt))))) out))
...
(defun make-comment (comment)
	   (make-instance 'comment :id (get-comment-id comment) :from (make-from (get-comment-from comment)) :message (get-comment-message comment) :created--time (get-comment-created--time comment )))

(defun make-comment-list (comments)
	   (let(( out '()))
	   (dolist (ct comments) (setf out (append out (list (make-comment ct))))) out))

(defun make-comments (comments)
	   (make-instance 'comments :data (make-comment-list (get-comments-data comments)) :count (get-comments-count comments)))
...
(defun make-application ( post ) ; param is post in list form
    (make-instance 'application :id (get-application-id post) :name (get-application-name  post) :namespace (get-application-namespace post)))
...
(defun make-post (post-raw)
    (make-instance 'post :id (get-post-id post-raw) :from (make-from (get-from post-raw)) :to (make-to-list (get-to-data (get-to post-raw))) :message (get-message post-raw) :message--tags (make-message--tag-list (get-message--tags post-raw)) :picture (get-picture post-raw) :link (get-link post-raw) :name (get-name post-raw) :caption (get-caption post-raw) :description (get-description post-raw) :source (get-source post-raw) :properties (make-property-list (get-properties-list post-raw)) :icon (get-icon post-raw) :actions (make-action-list post-raw) :privacy (make-privacy post-raw) :type (get-type post-raw) :likes (make-likes post-raw) :place (make-place (get-place post-raw)) :story (get-story post-raw) :story--tags (make-story--tag-list (get-story--tags post-raw)) :with--tags (make-with--tag-list (get-with-data post-raw)) :comments (make-comments (get-comments post-raw)) :object--id (get-object--id post-raw) :application (make-application post-raw) :created--time (get-created--time post-raw) :updated--time (get-updated--time post-raw)))

(defun make-post-list (post-list)
    (let ((out ()))
    (dolist (post post-list) (setf out (append out (list (make-post post))))) out))
